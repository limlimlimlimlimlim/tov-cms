const data = {
  id: 2,
  floorName: '2F',
  wing: [
    {
      id: 22,
      name: '전체',
      image: '/images/sample/map/2f/map.png',
      facility: [
        {
          id: 210,
          name: '사내피트니스',
          image: '/images/sample/facility/14.png',
          section: '/images/sample/map/2f/section1.png',
          phone: '02-2488-3368',
          address: '동관 2층 201호',
          description:
            '해머스트렝스 공식 인증센터! 100여 종의 해외 명품 머신 완비!',
          openingHours: '11:00 ~ 22:00',
          initial: 'ᄑᄐᄂᄉ, ᄋᄃ',
          floor: '2F',
          wing: '동관',
          floorId: 2,
          wingId: 21,
          x: 25,
          y: 4,
        },
        {
          id: 200,
          name: '사내병원',
          image: '/images/sample/facility/17.png',
          section: '/images/sample/map/2f/section4.png',
          phone: '02-2005-8900',
          address: '서관 2층 204호',
          description: '사내 지원들의 건겅을 위한 사내병원입니다.',
          openingHours: '09:00 ~ 19:30',
          initial: 'ㅂㅇ, ㅅㄴㅂㅇ',
          floor: '2F',
          wing: '서관',
          floorId: 2,
          wingId: 20,
          x: 305,
          y: 41,
        },
        {
          id: 201,
          name: '신한은행',
          image: '/images/sample/facility/16.png',
          section: '/images/sample/map/2f/section3.png',
          phone: '02-2452-1900',
          address: '서관 2층 203호',
          description:
            '인터넷뱅킹, 신한SOL뱅크, 외화 환전, 계좌조회, 펀드, 적금, 대출, 재형저축 상 품 안내',
          openingHours: '09:00 ~ 16:00',
          initial: 'ㅇㅎ, ᄉᄒ, ᄉᄒᄋᄒ',
          floor: '2F',
          wing: '서관',
          floorId: 2,
          wingId: 20,
          x: 266,
          y: 29,
        },
        {
          id: 202,
          name: '사내약국',
          image: '/images/sample/facility/15.png',
          section: '/images/sample/map/2f/section2.png',
          phone: '02-2221-6689',
          address: '서관 2층 202호',
          description:
            '직원의 건강을 위해 친절하게 복약 지도와 상담, 약 처방을 제공합니다.',
          openingHours: '10:00 ~ 22:00',
          initial: 'ㅅㄴㅇㄱ, ㅇㄱ',
          floor: '2F',
          wing: '서관',
          floorId: 2,
          wingId: 20,
          x: 243,
          y: 27,
        },
      ],
    },
    {
      id: 21,
      name: '동관',
      image: '/images/sample/map/2f/east/map.png',
      facility: [
        {
          id: 210,
          name: '사내피트니스',
          image: '/images/sample/facility/14.png',
          section: '/images/sample/map/2f/east/section1.png',
          phone: '02-2488-3368',
          address: '동관 2층 201호',
          description:
            '해머스트렝스 공식 인증센터! 100여 종의 해외 명품 머신 완비!',
          openingHours: '11:00 ~ 22:00',
          initial: 'ᄑᄐᄂᄉ, ᄋᄃ',
          floor: '2F',
          wing: '동관',
          floorId: 2,
          wingId: 21,
          x: 160,
          y: 0,
        },
      ],
    },
    {
      id: 20,
      name: '서관',
      image: '/images/sample/map/2f/west/map.png',
      facility: [
        {
          id: 200,
          name: '사내병원',
          image: '/images/sample/facility/17.png',
          section: '/images/sample/map/2f/west/section3.png',
          phone: '02-2005-8900',
          address: '서관 2층 204호',
          description: '사내 지원들의 건겅을 위한 사내병원입니다.',
          openingHours: '09:00 ~ 19:30',
          initial: 'ㅂㅇ, ㅅㄴㅂㅇ',
          floor: '2F',
          wing: '서관',
          floorId: 2,
          wingId: 20,
          x: 240,
          y: 25,
        },
        {
          id: 201,
          name: '신한은행',
          image: '/images/sample/facility/16.png',
          section: '/images/sample/map/2f/west/section2.png',
          phone: '02-2452-1900',
          address: '서관 2층 203호',
          description:
            '인터넷뱅킹, 신한SOL뱅크, 외화 환전, 계좌조회, 펀드, 적금, 대출, 재형저축 상 품 안내',
          openingHours: '09:00 ~ 16:00',
          initial: 'ㅇㅎ, ᄉᄒ, ᄉᄒᄋᄒ',
          floor: '2F',
          wing: '서관',
          floorId: 2,
          wingId: 20,
          x: 170,
          y: 0,
        },
        {
          id: 202,
          name: '사내약국',
          image: '/images/sample/facility/15.png',
          section: '/images/sample/map/2f/west/section1.png',
          phone: '02-2221-6689',
          address: '서관 2층 202호',
          description:
            '직원의 건강을 위해 친절하게 복약 지도와 상담, 약 처방을 제공합니다.',
          openingHours: '10:00 ~ 22:00',
          initial: 'ㅅㄴㅇㄱ, ㅇㄱ',
          floor: '2F',
          wing: '서관',
          floorId: 2,
          wingId: 20,
          x: 120,
          y: 0,
        },
      ],
    },
  ],
};
export default data;
